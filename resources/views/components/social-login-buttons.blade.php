    ...
    <div class="flex flex-col gap-3">
        ...
        @foreach ($providers as $provider => $label)
            <a href="{{ route('oauth.redirect', $provider) }}"
               class="flex items-center justify-center gap-2.5 w-full rounded-lg border border-white/20 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-white/10">
                @if ($provider === 'google')
