...
window.GXP = (function () {
        ...
        function fill(d) {
            ...
            // Type is fixed once a provider exists — changing it would strand its channel/guide
            // store. A disabled <select> still returns .value to payload(), so the type is still
            // submitted; we just prevent the user from changing it.
            const lockType = !!d.id;
            $('f-type').disabled = lockType;
            $('f-type').style.opacity = lockType ? '.55' : '';
            $('f-type').style.cursor = lockType ? 'not-allowed' : '';
            $('f-type').title = lockType ? 'Type can’t be changed for an existing provider.' : '';
            $('f-url').value = d.url ?? '';
